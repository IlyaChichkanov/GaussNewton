...
|---|---|
| `jacobian_fd_test.py` | finite differences of the residuals — also covers the gradient, the collocation path and, as a negative result, the loss of accuracy on a discontinuous input |
| `covariance_test.py` | single shooting (no constraints at all) and Monte-Carlo: spread of the estimates and coverage of the 95% intervals |
| `scaling_test.py` | the same problem written in different units — an exact reference for `cont_scale` |
...
| Test | Guards |
|---|---|
| `accumulated_test.py` | the accumulated `H`/`g` path against the dense one: matrices, step, covariance, and a full identification run |
| `adaptive_test.py` | `pred > 0` across regimes, identification, `N_shoot=5` on the attractor (the `rss_stall_tol` gate), early stop on single shooting, the collocation path |
| `collocation_test.py` | Radau tables, the IND property, a stiff simulation, identification, agreement with multiple shooting |
| `collocation_accum_test.py` | identification through `CollocationShootingAccum` |
| `gauss_newton_test.py` | the basic identification run |
| `sensitivity_test.py` | round trip of the flat integrator layout — the one thing that would otherwise silently permute axes |
| `systems_smoke_test.py` | every system in `commom_utils/systems.py` has consistent dimensions and compiles |
| `regression_test.py` | see below |

## The frozen reference

`regression_test.py` holds frozen `J`, `R`, `J_G`, `R_G`, `H`, `g` and `delta`
