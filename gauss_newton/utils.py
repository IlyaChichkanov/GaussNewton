...
def plot_solution(
    ...
    *,
    ...
    # ---- 1. Определение количества строк и весов высоты ----
    if is_single:
        total_rows = 1                     # график состояния от времени
    else:
        total_rows = 1                     # фазовая траектория
        if plot_xy:
            total_rows += n_obs          # по одной строке на каждое состояние
    ...
    # ---- 2. Верхняя панель (фазовая траектория или временной ряд для n_state==1) ----
    if not is_single:
        use_3d = n_state >= 3
        if use_3d:
            ax_traj = fig.add_subplot(gs[0, :], projection='3d')
    ...
    # ---- 3. Оси для временных рядов каждого состояния (только для n_state>1) ----
    axes_states = []   # axes_states[state_idx][batch_idx]
    ...
    def _plot_traj(ax, states, is_3d, **kwargs):
        if is_3d and states.shape[1] >= 3:
            ...
            # Этот случай теперь не вызывается при is_single=True,
            # оставлен для совместимости, если вдруг n_state==1 и не is_single (флаг не установлен)
            ax.plot(states[:, 0], np.zeros_like(states[:, 0]), **kwargs)
    ...
    # ---- Истинное решение (если требуется) ----
    if plot_true_solution and not is_single:
        # Для одномерного случая истинная траектория не поддерживается фазовой отрисовкой
        if not hasattr(problem, 'full_trajectory') or problem.full_trajectory is None:
            raise AttributeError("problem.full_trajectory is None")
        _, state_true_batches = problem.full_trajectory
        for state_true in state_true_batches:
            ...
            if is_single:
                ...
                if use_3d and state_measured.shape[1] >= 3:
                    ...
                    ax_traj.scatter(state_measured[:, 0], state_measured[:, 1],
                                    color='green', marker='x', s=15,
                                    label='Measurements' if batch == 0 else "")
                # plot_xy измерения добавляются в том же блоке, где и временные ряды
                if plot_xy:
                    for state_idx in range(n_obs):
                        axes_states[state_idx][batch].scatter(t_meas, state_measured[:, state_idx],
                                                              color='green', marker='x', s=15)

    # Легенда для верхней панели (только если не одномерный режим)
    if not is_single:
        ax_traj.legend(fontsize=fontsize)
