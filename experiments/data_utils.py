...
#         print(f"Loaded {len(self.df_data)} rows from {path}")
#         print(f"Columns: {list(self.df_data.columns)}")

#     def add_batch(self, dataname: str, time_ns: bool = False, use_jax_interp=True, scale_coef=1):
#         if dataname not in self.df_data.columns:
#             print(f"Available columns: {list(self.df_data.columns)}")
#             raise ValueError(f"Column '{dataname}' not found in {self.path}")
        
#         time = self.df_data["time"].values.copy().astype(float)
#         values = self.df_data[dataname].values.copy().astype(float) * scale_coef
...
#         if len(time) == 0:
#             raise ValueError(f"No valid data for {dataname}")
...
#         print(f"Queued {dataname}: time [{time[0]:.3f}, {time[-1]:.3f}], "
#               f"values [{values.min():.3f}, {values.max():.3f}]")
    
#     def process_all(self):
#         """Обрабатывает все данные, используя минимальное время как t0"""
#         if not self._pending_batches:
#             print("No data to process")
#             return
        
#         # Находим минимальное время среди всех данных
#         all_start_times = [batch['time_original'][0] for batch in self._pending_batches]
#         self.t0 = min(all_start_times)
#         print(f"\nSet t0 = {self.t0} (minimum of all start times)")
        
#         # Обрабатываем все batches
#         for batch in self._pending_batches:
#             time_normalized = batch['time_original'] - self.t0
            
#             # Создаем интерполятор
#             if batch['use_jax_interp']:
#                 f_interp = create_jax_interpolator(time_normalized, batch['values'], method='linear')
#                 print("use jax")
#             else:
#                 f_interp = interp1d(time_normalized, batch['values'], kind='linear', 
#                                    bounds_error=False, fill_value='extrapolate')
            
#             # Сохраняем данные
#             self.data[batch['name']] = {
#                 'interpolator': f_interp,
#                 'time_original': batch['time_original'],
...
#             # Обновляем общие границы времени
#             self.t1 = max(self.t1, time_normalized[0])
#             self.t2 = min(self.t2, time_normalized[-1])
            
#             print(f"Processed {batch['name']}: "
#                   f"original [{batch['time_original'][0]:.3f}, {batch['time_original'][-1]:.3f}], "
#                   f"normalized [{time_normalized[0]:.3f}, {time_normalized[-1]:.3f}]")
        
#         self._pending_batches.clear()
#         print(f"Common time range: [{self.t1:.3f}, {self.t2:.3f}]")

#     def get_f_interp(self, dataname: str):
#         """Возвращает функцию интерполяции"""
#         if dataname not in self.data:
#             raise ValueError(f"Data '{dataname}' not loaded. Available: {list(self.data.keys())}")
#         return self.data[dataname]['interpolator']

#     def get_time(self, dataname: str):
#         """Возвращает нормализованное время"""
#         if dataname not in self.data:
#             raise ValueError(f"Data '{dataname}' not loaded")
#         return self.data[dataname]['time']
    
#     def get_original_time(self, dataname: str):
#         """Возвращает оригинальное время"""
#         if dataname not in self.data:
#             raise ValueError(f"Data '{dataname}' not loaded")
#         return self.data[dataname]['time_original']
    
#     def get_raw_data(self, dataname: str):
#         """Возвращает значения и нормализованное время"""
#         if dataname not in self.data:
#             raise ValueError(f"Data '{dataname}' not loaded")
#         return self.data[dataname]['values'], self.data[dataname]['time']
    
#     def get_data_at_time(self, dataname: str, t):
#         """Получить интерполированное значение в заданный момент времени"""
#         f = self.get_f_interp(dataname)
#         return float(f(t))
    
#     def get_common_time_range(self):
#         """Возвращает общий временной диапазон для всех данных"""
#         return self.t1, self.t2
...
#         for ax, (name, data_dict) in zip(axes, self.data.items()):
#             ax.plot(data_dict['time'], data_dict['values'], label=name)
#             ax.set_xlabel('Time (normalized)')
#             ax.set_ylabel(name)
#             ax.legend()
#             ax.grid(True, alpha=0.3)
